...
(function () {
    ...
    DialogEl.prototype._init = function () {
        ...
        // close dialog
        var self = this;
        this.el.querySelector('button.action--close').addEventListener('click', function () {
            self.close();
        });
        // init Draggabilly
    ...
    DialogEl.prototype._onDragMove = function (event, pointer, moveVector) {
        ...
        if (this._outOfBounds()) {
            dynamics.css(this.moverEl, {
                rotate: 0,
                scale: 1,
                opacity: 1
            });
            dynamics.css(this.innerEl, {
                translate: 0
            });
    ...
    DialogEl.prototype._initDraggabilly = function () {
        ...
        this.draggie.on('dragStart', function (event, pointer, moveVector) {
            self._onDragStart(event, pointer, moveVector);
        });
        ...
        this.draggie.on('dragEnd', function (event, pointer, moveVector) {
            self._onDragEnd(event, pointer, moveVector);
        });
    };
    ...
    var $usernameField = $('input#user-login'),
        $passwordField = $('input#user-pass'),
    ...
    function submitLogin() {
        ...
        var loginURL = "functions/generate_auth_cookie.php";
        var data = new Array();

        data['username'] = $usernameField.val();
        data['password'] = $passwordField.val();
        $('#login-button').html('<i class="fa fa-spin fa-circle-o-notch"></i> 登陆中...');
        ...
        jQuery.ajax({
            ...
            success: function (results) {
                if (results['status'] == "ok") {
                    if (!results['error']) {
                        $('#login-button').html('<i class="fa fa-check"></i> 登录成功');
                        setCookie('chat-logged-in', results['user']['id'] + "|" + results['cookie'], 100000);
                        setTimeout(function () {
                            $('#login-dialog').fadeOut(500, function () {
                                $('#login-dialog').remove();
                    ...
                    $('#login-button').html('<i class="fa fa-times"></i> 登录失败，重试');
                }
            }
        });
    }
